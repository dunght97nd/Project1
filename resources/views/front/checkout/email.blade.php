                            ...
                            @foreach($order->orderDetails as $orderDetail)
                                <tr>
                                    <td style="border-top: 1px solid #dee2e6; padding: 5px 0;">
                                        {{ $orderDetail->productDetail->product->name . ' ($'.number_format($orderDetail->amount,2).'x'. $orderDetail->qty . ')'}}
                                    </td>
                                    <td style="border-top: 1px solid #dee2e6; padding: 5px 20px 5px 0; text-align: right;">
                                        ${{ number_format($orderDetail->amount * $orderDetail->qty, 2) }}
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
                                ...
                                @if(Session::get('coupon'))
                                    @foreach(Session::get('coupon') as $key => $cou)
                                        ...
                                        @if($cou['coupon_condition'] == 1)
                                            <tr>
                                                <td class="">Discount({{$cou['coupon_number']}}%):</td>
                                                <td class="pr-3 text-right" style="text-align: right; padding-right: 20px;">
                                                -${{number_format($subtotal * $cou['coupon_number'] / 100, 2)}}
                                                </td>
                                            </tr>
                                            <tr style="font-size: 18px;">
                                                <td><b>TOTAL</b></td>
                                                <td class="pr-3 text-right" style="text-align: right; padding-right: 20px;">
                                                    <b>${{number_format($subtotal - $subtotal * $cou['coupon_number'] / 100, 2)}}</b>
                                                </td>
                                            </tr>
                                        @elseif($cou['coupon_condition'] == 2)
                                            <tr>
                                                <td class="">Discount(${{$cou['coupon_number']}}):</td>
                                                <td class="pr-3 text-right" style="text-align: right; padding-right: 20px;">
                                                -${{number_format($cou['coupon_number'], 2)}}
                                                </td>
                                            </tr>
                                            <tr style="font-size: 18px;">
                                                <td><b>TOTAL</b></td>
                                                <td class="pr-3 text-right" style="text-align: right; padding-right: 20px;">
                                                    <b>${{number_format($subtotal - $cou['coupon_number'], 2)}}</b>
                                                </td>
                                            </tr>
                                        @endif
                                    @endforeach
